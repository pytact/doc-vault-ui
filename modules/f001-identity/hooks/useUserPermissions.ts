...
interface UseUserPermissionsParams {
  ...
  targetUserRole?: "superadmin" | "familyadmin" | "member" | null; // Target user's role
}
...
export function useUserPermissions(
  ...
  const canManageUserRoles = useMemo(() => {
    ...
    // FamilyAdmin cannot manage roles for SuperAdmin users
    if (currentUserRole === "familyadmin" && targetUserRole === "superadmin") {
      return false;
    }
    
    return currentUserRole === "superadmin" || currentUserRole === "familyadmin";
  }, [currentUserRole, targetUserStatus, familyStatus, targetUserRole]);
